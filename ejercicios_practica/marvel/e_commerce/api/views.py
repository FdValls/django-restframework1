...
def comic_list_filtered_api_view(request):
    filtered_comics = Comic.objects.filter(price__gt=5.00)
...
def comic_create_api_view(request):
    if request.method == 'POST':
        try:
            # Intenta obtener datos JSON desde el cuerpo de la solicitud
            data = json.loads(request.body.decode('utf-8'))

            # Imprime el cuerpo de la solicitud
            print(f"Cuerpo de la solicitud: {data}")

            # Extrae el campo marvel_id de los datos
            _marvel_id = data.get('marvel_id', None)
            print(f"que tengo en _marvel_id: {_marvel_id}")

            # Verifica si marvel_id es nulo
            if _marvel_id is None:
                raise ValueError("marvel_id no puede ser nulo.")

            # Resto de tu código...
            _instance, _created = Comic.objects.get_or_create(
                marvel_id=_marvel_id,
                defaults=data  # Utiliza directamente los datos obtenidos
            )

            if _created:
                return JsonResponse(
                    data=model_to_dict(_instance),
                    status=201
                )
            else:
                return JsonResponse(
                    data={"marvel_id": "Ya existe un comic con ese valor, debe ser único."},
                    status=400
                )
